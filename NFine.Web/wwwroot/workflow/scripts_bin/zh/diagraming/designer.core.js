...
var Designer = {
    ...
    initialize: {
        ...
        initShapes: function() {
           ...
           // $("#shape_panel").append("<div class='panel_container panel_collapsed'><h3 class='panel_title search'><input id='shape_search' placeholder='搜索'  title='搜索ProcessOn图形和网络图标，搜索网络图标建议使用英文单词，搜索更精准。' title_pos='right'/></h3><div id='panel_search' class='content'></div></div>");
            ...
            $("#shape_search").unbind("keydown").bind("keydown",
            function(q) {
                if (q.keyCode == 13) {
                    $("#panel_search").empty();
                    ...
                    if (n.trim() != "") {
                        ...
                        function i() {
                            ...
                            $.getJSON("https://api.iconfinder.com/v2/icons/search?query=" + n + "&offset=" + o + "&count=24&minimum_size=64&vector=false&premium=false",
                            function(w) {
                                if (w.total_count == 0) {
    ...
    hotkey: {
        init: function() {
            ...
            $(window).unbind("keyup.hotkey").bind("keyup.hotkey",
            function(b) {
                if (b.keyCode == 91) {
                    b.preventDefault();
    ...
    op: {
        init: function() {
            ...
            a.unbind("mousemove.operate").bind("mousemove.operate",
            function(g) {
        ...
        groupShapeChangable: function() {
            ...
            function(f) {
                ...
                Designer.op.groupDashboard(h, c, b,
                function(e) {
                    if (a.name != e) {
        ...
        shapeMultiSelectable: function() {
            ...
            b.unbind("mousedown.multiselect").bind("mousedown.multiselect",
            function(e) {
                var d = null;
                if (!e.ctrlKey) {
                    Utils.unselect()
        ...
        shapeEditable: function(a) {
            var b = $("#designer_canvas");
            b.unbind("dblclick.edit").bind("dblclick.edit",
            function(c) {
        ...
        shapeLinkable: function(c, a) {
            ...
            var b = $("#canvas_container");
            b.unbind("mousedown.link").bind("mousedown.link",
            function(h) {
        ...
        linkerEditable: function(b) {
            var a = $("#designer_canvas");
            a.unbind("dblclick.edit_linker").bind("dblclick.edit_linker",
            function() {
        ...
        textCreatable: function() {
            var b = $("#designer_canvas");
            var a = $("#canvas_container");
            a.unbind("mousedown.create_text").bind("mousedown.create_text",
            function(f) {
                var d = null;
                if (!f.ctrlKey) {
                    Utils.unselect()
        ...
        canvasDraggable: function() {
            ...
            a.off("mousedown.drag_canvas").on("mousedown.drag_canvas",
            function(d) {
                var c = $("#designer_layout").scrollTop();
                var b = $("#designer_layout").scrollLeft();
                a.on("mousemove.drag_canvas",
                function(f) {
                ...
                $(document).off("mouseup.drag_canvas").on("mouseup.drag_canvas",
                function(e) {
                    a.off("mousemove.drag_canvas");
                    $(document).off("mouseup.drag_canvas")
                })
            });
            $(document).off("keyup.drag_canvas").on("keyup.drag_canvas",
            function(b) {
        ...
        canvasFreeDraggable: function() {
            var a = $("#canvas_container");
            a.css("cursor", "url(/themes/default/images/diagraming/cursor_hand.png) 8 8, auto");
            a.unbind("mousedown.drag_canvas").bind("mousedown.drag_canvas",
            function(d) {
                var c = $("#designer_layout").scrollTop();
                var b = $("#designer_layout").scrollLeft();
                a.bind("mousemove.drag_canvas",
                function(f) {
                    var e = f.pageX - d.pageX;
                    var g = f.pageY - d.pageY;
                    $("#designer_layout").scrollLeft(b - e);
        ...
        linkDashboard: function(g) {
            ...
            if ($("#panel_" + b).length != 0) {
                ...
                i.find(".link_shape_icon").unbind().bind("mousedown",
                function(q) {
                    q.stopPropagation();
                    ...
                    Designer.op.groupDashboard(p, o, n,
                    function(t) {
                        j(t);
                        i.hide();
                ...
                i.children(".dashboard_box").unbind().bind("click",
                function() {
                    i.hide();
                    $(document).unbind("mousedown.dashboard");
                    var o = $(this);
    ...
    clipboard: {
        ...
        brush: function() {
            ...
            UI.showTip("选择目标图形并使用格式刷样式，Esc取消", "left",
            function() {
    ...
    painter: {
        ...
        renderShape: function(j) {
            ...
            n.lineJoin = "round";
            this.renderShapePath(n, j, false,
            function() {
                var q = j.id;
...
var Model = {
    ...
    create: function(e, b, g) {
        ...
        c.props = $.extend(true, {},
        Schema.shapeDefaults.props, c.props);
...
var Utils = {
    ...
    getShapeLineStyle: function(a, b) {
        if (b == false || !Model.define.theme || !Model.define.theme.shape) {
            return $.extend({},
            Schema.shapeDefaults.lineStyle, a)
        } else {
            return $.extend({},
            Schema.shapeDefaults.lineStyle, Model.define.theme.shape.lineStyle, a)
        }
    },
    getLinkerLineStyle: function(a, b) {
        if (b == false || !Model.define.theme || !Model.define.theme.linker) {
            return $.extend({},
            Schema.linkerDefaults.lineStyle, a)
        } else {
            return $.extend({},
            Schema.linkerDefaults.lineStyle, Model.define.theme.linker.lineStyle, a)
        }
    },
    getShapeFontStyle: function(b, a) {
        if (a == false || !Model.define.theme || !Model.define.theme.shape) {
            return $.extend({},
            Schema.shapeDefaults.fontStyle, b)
        } else {
            return $.extend({},
            Schema.shapeDefaults.fontStyle, Model.define.theme.shape.fontStyle, b)
        }
    },
    getLinkerFontStyle: function(b, a) {
        if (a == false || !Model.define.theme || !Model.define.theme.linker) {
            return $.extend({},
            Schema.linkerDefaults.fontStyle, b)
        } else {
            return $.extend({},
            Schema.linkerDefaults.fontStyle, Model.define.theme.linker.fontStyle, b)
        }
    },
    getShapeFillStyle: function(a, b) {
        if (b == false || !Model.define.theme || !Model.define.theme.shape) {
            return $.extend({},
            Schema.shapeDefaults.fillStyle, a)
        } else {
            return $.extend({},
            Schema.shapeDefaults.fillStyle, Model.define.theme.shape.fillStyle, a)
        }
    },
